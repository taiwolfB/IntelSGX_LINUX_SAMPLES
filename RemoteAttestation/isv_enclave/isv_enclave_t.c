...
static sgx_status_t SGX_CDECL sgx_put_secret_data(void* pms)
{
	...
	ms_put_secret_data_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_put_secret_data_t), ms, sizeof(ms_put_secret_data_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_p_secret = __in_ms.ms_p_secret;
	uint32_t _tmp_secret_size = __in_ms.ms_secret_size;
	...
	_in_retval = put_secret_data(__in_ms.ms_context, _in_p_secret, _tmp_secret_size, _in_gcm_mac);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
